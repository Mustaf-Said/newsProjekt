...
import {
  Home, Newspaper, Trophy, ShoppingBag, Car, Building2, Map,
  User, LogOut, Menu, X, ChevronDown, Sun, Moon, Search,
                      ...
                      <DropdownMenuContent align="start" className="w-48">
                        {item.children.map((child) => (
                          <DropdownMenuItem key={child.page} asChild>
                            <Link to={createPageUrl(child.page)} className="cursor-pointer">
                              {child.label}
                            </Link>
                          </DropdownMenuItem>
                        ))}
                ...
                {user && (
                  <>
                    <div className="border-t border-[var(--border)] pt-2 mt-2">
                      <Link to={createPageUrl("Dashboard")} onClick={() => setMobileOpen(false)} className="flex items-center gap-2 px-3 py-2.5 text-sm rounded-lg hover:bg-[var(--bg-secondary)]">
                        <LayoutDashboard className="w-4 h-4" /> Dashboard
                      </Link>
                      {user.role === "admin" && (
                        <Link to={createPageUrl("AdminPanel")} onClick={() => setMobileOpen(false)} className="flex items-center gap-2 px-3 py-2.5 text-sm rounded-lg hover:bg-[var(--bg-secondary)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              <div>
                ...
                <div className="space-y-2">
                  <Link to={createPageUrl("LocalNews")} className="block text-sm hover:text-amber-400 transition-colors">Local News</Link>
                  <Link to={createPageUrl("WorldNews")} className="block text-sm hover:text-amber-400 transition-colors">World News</Link>
                  <Link to={createPageUrl("FootballNews")} className="block text-sm hover:text-amber-400 transition-colors">Football News</Link>
                </div>
              </div>
              <div>
                <h4 className="text-white font-semibold mb-3 text-sm uppercase tracking-wider">Marketplace</h4>
                <div className="space-y-2">
                  <Link to={createPageUrl("MarketplaceCars")} className="block text-sm hover:text-amber-400 transition-colors">Cars</Link>
                  <Link to={createPageUrl("MarketplaceHouses")} className="block text-sm hover:text-amber-400 transition-colors">Houses</Link>
                  <Link to={createPageUrl("MarketplaceLand")} className="block text-sm hover:text-amber-400 transition-colors">Land</Link>
                </div>
              </div>
              <div>
                <h4 className="text-white font-semibold mb-3 text-sm uppercase tracking-wider">Account</h4>
                <div className="space-y-2">
                  <Link to={createPageUrl("Dashboard")} className="block text-sm hover:text-amber-400 transition-colors">Dashboard</Link>
                  {!user && (
