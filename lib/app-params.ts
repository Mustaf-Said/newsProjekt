...
const storage: Storage = windowObj.localStorage as Storage;

const toSnakeCase = (str: string) => str.replace(/([A-Z])/g, "_$1").toLowerCase();

const getAppParamValue = (paramName: string, { defaultValue = undefined, removeFromUrl = false }: { defaultValue?: string; removeFromUrl?: boolean } = {}) => {
  if (isNode) {
    return defaultValue;
  }
